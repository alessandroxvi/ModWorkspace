...
package net.mcreator.thexvimod.world.biome;
...
public class BiomeXenon extends ElementsTheXVImod.ModElement {
	@GameRegistry.ObjectHolder("thexvimod:xenon")
	public static final BiomeGenCustom biome = null;
	public BiomeXenon(ElementsTheXVImod instance) {
		super(instance, 28);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Xenon").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-6723841).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("xenon");
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == Blocks.YELLOW_GLAZED_TERRACOTTA.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, Blocks.YELLOW_GLAZED_TERRACOTTA.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.VINE.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.YELLOW_GLAZED_TERRACOTTA.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), Blocks.BLACK_GLAZED_TERRACOTTA.getDefaultState());
							}
						}
								...
								for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										this.setBlockAndNotifyAdequately(world,
												...
												BlockCarbon.block.getDefaultState());
									}
								}
							}
						}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR
					...
					|| blockType == Blocks.YELLOW_GLAZED_TERRACOTTA.getDefaultState().getBlock()
					|| blockType == Blocks.GRASS.getDefaultState().getBlock() || blockType == Blocks.DIRT.getStateFromMeta(0).getBlock();
		}
